...
module Jekyll
  ...
  class CategoryPage < Page
    ...
    def initialize(template_path, name, site, base, category_dir, category, tags)
      @site  = site
      @base  = base
      @dir   = category_dir
      @name  = name
  ...
  class CategoryFeed < CategoryPage
    ...
    def initialize(site, base, category_dir, category, tags)
      ...
      super(template_path, 'atom.xml', site, base, category_dir, category, tags)
  ...
  class Site
    ...
    #
    #  +category+ is the category currently being processed.
    def write_category_index(category, tags)
      puts self.config['category_dir']
      target_dir = GenerateCategories.category_dir(self.config['category_dir'], category, tags)
      index      = CategoryIndex.new(self, self.source, target_dir, category, tags)
      if index.render?
      ...
      feed = CategoryFeed.new(self, self.source, target_dir, category, tags)
      if feed.render?
  ...
  module Filters
    ...
    def category_links(categories)
      ...
      categories = categories.sort!.map do |category|
        category_dir = GenerateCategories.category_dir(base_dir, category, tags)
